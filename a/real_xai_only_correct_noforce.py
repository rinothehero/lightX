...
def train_epoch(model, dataloader, optimizer, args, device, tokenizer):
    ...
    for batch in dataloader:
        ...
        xai_per_sample = (
            diff2[torch.arange(diff2.size(0)), true_labels, :] * mask
        ...
        # • same_mask: both MLP & BERT match the true label
        same_mask = (mlp_preds == bert_preds) & (mlp_preds == true_labels)  # [B]
        # • corr_mask: only MLP matches the true label (BERT wrong)
        corr_mask = (mlp_preds == true_labels) & (mlp_preds != bert_preds)  # [B]

        # base_alpha: α₁ if same_mask, else α₂ if corr_mask, else α₃
        base_alpha = torch.where(
            same_mask, args.alpha1,
            torch.where(corr_mask, args.alpha2, args.alpha3)
        )  # [B]
                # • bert_correct_mask: BERT predicted the true label
        #bert_correct_mask = (bert_preds == true_labels)  # [B]

        # final α: use base_alpha only when bert_correct_mask, else force α=1 → only CE loss
        #alpha = torch.where(
        #    bert_correct_mask,
        #    base_alpha,
        #    torch.ones_like(base_alpha)
        #)  # [B]
       
        # 5) Combine CE & XAI losses with per‐sample weighting
        loss_per_sample = base_alpha * ce_per_sample + (1 - base_alpha) * xai_per_sample  # [B]
        ...
        #if(true_labels == bert_preds) :
        #    xai_per_sample = (diff2[:, true_labels, :] * mask).sum(dim=1) / token_counts
        #     # adaptive alpha
        #    mlp_preds = logits.argmax(dim=1)
...
def main():
    ...
    torch.save(state, "./models/only_correct_noforce_09_09.pt")
